...
class Visual_Application {
    ...
    Visualize_element_of_list(name, model, color) {
        var html = '<div class="pb-4 listview__item"><label class="pl-0 pr-4 custom-control custom-control--char"><input class="custom-control-input" type="checkbox"><span class="custom-control--char__helper" style="background-color:' + color + '"><i></i></span></label><div class="listview__content"><div class="listview__heading">' + name + '</div><p>' + model + ' model</p></div><button class="btn btn-secondary btn--icon-text pop-function btn-load mr-3" rel="popover" data-toggle="popover" data-trigger="hover" title="Information" data-content="To load the values of the parameters, the function it must be selected"><i class="zmdi zmdi-refresh"></i>Load</button></span><button class="btn btn-secondary btn--icon-text pop-function btn-delete" rel="popover" data-toggle="popover" data-trigger="hover" title="Information" data-content="To eliminate, the function it must be selected"><i class="zmdi zmdi-delete"></i>Delete</button></span></div>';
        // var html = '<div class="pb-4 listview__item"><label class="pl-0 pr-4 custom-control custom-control--char"><input class="custom-control-input" type="checkbox"><span class="custom-control--char__helper" style="background-color:' + color + '"><i></i></span></label><div class="listview__content"><div class="listview__heading">' + name + '</div><p>' + model + ' model</p></div><label class="custom-control custom-checkbox align-self-start"><i class="zmdi zmdi-edit zmdi-hc-2x"></i></span></label><label class="custom-control custom-checkbox align-self-start"><button class="btn btn-secondary btn--icon"><i class="zmdi zmdi-search"></i></button></span></label></div>';
    ...
    static Add_Matrix(html, html_append, order, matrix_collection, id, vector) {
        ...
        if (vector) this.Initialize_Matrix(matrix, this.Fill_Initial_Data_Vector(1, order));
    ...
    static Build_Visual_Scenario(time_size, nssc_scenario, matrix_collection, deme_vector_collection, sampling_vector, order, type, number_of_events) {
        this.Initialize_Matrix(sampling_vector, Visual_Application.Fill_Initial_Data_Vector(0, order));
